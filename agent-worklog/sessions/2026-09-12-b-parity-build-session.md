...
- PROVEN (2026-09-13, direct binary experiment): the 3 death-test failures are STALE TESTS, not a product bug. Built `ovms.exe --list_models` (no repo path) exits 0 and lists `C:\llm\models` — the substrate now has a DEFAULT model repository path, so the old "must die with OVMS_EX_USAGE" contract in `ovmsconfig_test.cpp:359,811,948` is outdated. `--pull` without repo path likewise proceeds into the default path (tried cloning, failed only on network 404), not usage validation. Fix belongs to the tests (update expectations for the default path), not to product code and not to `src/llm`. The stress-test crash (0xC0000005) is a SEPARATE unresolved item, not covered by this proof.
