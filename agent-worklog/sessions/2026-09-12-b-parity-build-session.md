...
- CORRECTION (2026-09-13, maintainer commits a3957461/7fcb024f + proof): the "stale tests / compiled-in default" verdict above was WRONG. Root cause is environmental: host USER env has `OVMS_MODEL_REPOSITORY_PATH=C:\llm\models`, which OVMS intentionally uses as the default `--model_repository_path`, making those three inputs valid. Maintainer gate `tests/windows/gemmamonster_rc2_ovms_test_gate.ps1` clears the var: all 3 negative CLI tests PASS (isolated 3/3 and again inside the full run). Product code and tests are both correct; my earlier claim is retracted.
- Gate run (2026-09-13): negative-CLI gate PASS (3/3). Full suite minus upstream-known-sporadic `ConfigChangeStressTestSingleModel.ChangeToEmptyConfigInference` (CVS-176244) still CRASHED 0xC0000005, this time inside sibling `ConfigChangeStressTestAsync.ChangeToEmptyConfigAsyncInference` — same config-change/async-stress family. Crash family is broader than the single excluded test; root cause still open for a source/runtime session. No package produced; READY_FOR_ACCEPTANCE stays NO.
