...
$messages = @(
  [ordered]@{ role = "user"; content = $taskText }
)
...
for ($iter = 1; $iter -le $maxIter -and -not $stopSweep; $iter++) {
  ...
  $start = (Get-Content "$ev\server.stdout.log" | Measure-Object -Line).Lines
  ...
  $total = (Get-Content "$ev\server.stdout.log" | Measure-Object -Line).Lines
  Get-Content "$ev\server.stdout.log" | Select-Object -Skip $start | Set-Content "$itd\server-window.log"
  $m = Select-String -Path "$itd\server-window.log" -Pattern "will not be applied|due to JSON|StructuredOutput|xgrammar|XGrammar|guided generation|Guided|validation failure|Validation"
  if ($m) { $m | ForEach-Object { $_.Line } | Set-Content "$itd\server-guided-lines.txt" } else { "NO_MATCHES" | Set-Content "$itd\server-guided-lines.txt" }
  ...
  if ($null -eq $reasoning) { $reasoning = $msg.reasoning }
  if ($null -eq $reasoning) { $reasoning = "" }
  ...
  $isToolTurn = $hasCalls
  if ($isToolTurn) { $toolTurns++ }
  $hasOpen = ($content -match "<call:")
  $hasBare = ($content -match "(^|[^<:a-zA-Z_])call:[a-zA-Z_]") -or ($content -match "^call:[a-zA-Z_]")
  ...
  $row = [ordered]@{
    ...
    tool_choice = "auto"
    parallel_tool_calls = "not-set(default)"
    sampling = "temperature=0,max_tokens=256"
    structured_output_config = "absent"
    validation_msgs = ((Get-Content "$itd\server-guided-lines.txt" -Raw) -replace "`r`n", " / ")
    reasoning_present = ($reasoning -ne "")
    reasoning_excerpt = "$reasoning".Substring(0, [Math]::Min(200, "$reasoning".Length))
    tool_calls_count = $tcs.Count
    finish_reason = $choice.finish_reason
    tool_names = $names
    tool_arguments = ($argList -join " | ")
    sig = $sig
    literal_call_open = $hasOpen
    literal_bare_call = $hasBare
    repeat_prev_sig = $isRepeat
    empty_turn = $isEmpty
    finish_length = $isLength
    content_excerpt = "$content".Substring(0, [Math]::Min(300, "$content".Length))
    is_checkpoint = ($checkpoints -contains $toolTurns)
  }
  $results += [pscustomobject]$row
  ($row | ConvertTo-Json -Depth 6) | Set-Content "$itd\summary.json"

  Write-Output ("iter={0} toolturn={1} finish={2} calls={3} names=[{4}] args=[{5}] <call:={6} barecall={7} repeat={8} empty={9} len={10} ptok={11} ctok={12}" -f $iter, $toolTurns, $choice.finish_reason, $tcs.Count, $names, ($argList -join " | "), $hasOpen, $hasBare, $isRepeat, $isEmpty, $isLength, $resp.usage.prompt_tokens, $resp.usage.completion_tokens)

  # classify first deviation
  $dev = $null
  if ($hasOpen) { $dev = "DIALECT_DRIFT_CALL_TAG" }
  elseif ($hasBare) { $dev = "DIALECT_DRIFT_BARE_CALL" }
  ...
  elseif (($reasoning -match "echo|sweep-item|call") -and (-not $hasCalls)) { $dev = "REASONING_TOOL_HANDOFF_FAILURE" }
  if ($dev -and (-not $firstDeviation)) {
    ...
    if ($dev -like "DIALECT_DRIFT*" -or $dev -eq "EMPTY_POST_TOOL_TURN" -or $dev -eq "GRAMMAR_THRASH_LENGTH") { $stopSweep = $true }
    if ($dev -eq "REPEATED_CANONICAL_TOOL_CALL") { Write-Output "A-before-D policy: continuing a few more turns to check repetition -> retry pressure -> drift" }
  }
  # A-before-D policy: continue up to 6 more iters after first A
  ...
  if ($hasCalls) {
    $prevSig = $sig
    $asst = [ordered]@{ role = "assistant"; content = $msg.content; tool_calls = @($tcs | ForEach-Object { [ordered]@{ id = $_.id; type = $_.type; function = [ordered]@{ name = $_.function.name; arguments = $_.function.arguments } } }) }
    $messages += $asst
    foreach ($tc in $tcs) {
      $echoed = ""
  ...
  if ($iter -ge $maxIter) { $stopSweep = $true }
}

$results | ConvertTo-Json -Depth 6 | Set-Content "$td\sweep-summary.json"
$results | Format-Table iter, tool_turn, finish_reason, tool_calls_count, tool_names, tool_arguments, literal_call_open, repeat_prev_sig, empty_turn, prompt_tokens, completion_tokens | Out-String | Set-Content "$td\sweep-table.txt"
...
Write-Output "=== SWEEP DONE: toolturns=$toolTurns firstDeviation=$((Get-Content "$td\first-deviation.json" -Raw)) ==="
Get-Content "$td\sweep-table.txt"
