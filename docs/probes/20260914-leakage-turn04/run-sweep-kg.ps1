...
$td = "$ev\deep-dive\sweep-kg"
New-Item -ItemType Directory -Path $td -Force | Out-Null
$logf = "$ev\deep-dive\prefixfalse-graph\server.stdout.log"
"graph=prefixfalse max_num_seqs=256 device=GPU VLM_CB temp=1.0 top_p=0.95 top_k=64 max_tokens=256 parallel_tool_calls=false" | Set-Content "$td\conditions.txt"
