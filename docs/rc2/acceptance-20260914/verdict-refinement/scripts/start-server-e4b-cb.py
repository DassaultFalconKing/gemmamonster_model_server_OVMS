...
EVID = Path(r'C:\Users\testc\AppData\Local\Temp\opencode\rc2-roundtrip-forensics\server-e4b-cb')
EVID.mkdir(parents=True, exist_ok=True)
MODEL = 'C:/llm/models/OpenVINO/gemma-4-E4B-it-int4-ov'
...
graph_text = '''# RC2 E4B/GPU/CB executor-swap experiment 2026-09-14.
...
try:
    ...
    for i in range(600):
        ...
        time.sleep(1)
    print(f"READY={ready}")
    (EVID/'ready.json').write_text(json.dumps({"ready":ready,"pid":p.pid,"model":MODEL,"pipeline":"VLM_CB/GPU"}))
finally:
    print(f"EVID: {EVID}")
