...
## 12. Продолжение: вердикт уточнён расследованием

См. [`verdict-refinement/README.md`](verdict-refinement/README.md): та же probe
против heretic-26B на том же RC2 — `OVERALL: PASS` (turn2 продолжается, утечки
`<eos>` нет). Смерть turn2 на E4B — весовая неспособность модели (H1
подтверждена), механика продукта реабилитирована; E4B на CB падает с SDPA-
ошибкой и на GPU. Буква вердикта для E4B-конфигурации сохраняется, атрибуция
меняется на model capability.
