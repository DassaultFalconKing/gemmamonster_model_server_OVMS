...
class NativeValueParser {
    ...
    bool writeJsonScalar(const std::string& token) {
        ...
        if (first == '"')
            ...
            type = rapidjson::kTrueType;
        else if (first == 'f')
            type = rapidjson::kFalseType;
        else if (first == 'n')
            type = rapidjson::kNullType;
        else if (!(first == '-' || std::isdigit(static_cast<unsigned char>(first))))
            return false;

        return writer.RawValue(normalized->data(),
            static_cast<rapidjson::SizeType>(normalized->size()), type);
    }
    ...
    bool parseJsonString() {
        ...
        while (pos < input.size()) {
            ...
            if (c == '"')
                return writeJsonScalar(input.substr(start, pos - start));
        }
        return false;
    }
...
std::optional<size_t> Gemma4ToolParser::findMatchingContainerEnd(
...
bool Gemma4ToolParser::parseInContentState() {
    ...
    streamingPosition = toolCallStartTagPos + TOOL_CALL_START_TAG.size();
...
bool Gemma4ToolParser::parseInToolCallState() {
    ...
    currentCallValid = saneToolName(toolName);
...
bool Gemma4ToolParser::parseToolCallParametersState() {
    ...
    size_t malformedEndTag = std::string::npos;
    auto closePos = findMatchingContainerEnd(
        streamingContent, openPos, currentArgsOpen, currentArgsClose, malformedEndTag);
    if (!closePos.has_value()) {
        if (malformedEndTag != std::string::npos) {
            SPDLOG_LOGGER_WARN(llm_calculator_logger,
                "Gemma4 malformed tool arguments bounded by <tool_call|>; dropping current call");
            streamingPosition = malformedEndTag + TOOL_CALL_END_TAG.size();
            currentState = State::AfterToolCall;
            currentCallValid = false;
            toolCall = {};
            return true;
    ...
    const std::string argumentsBody = streamingContent.substr(
        streamingPosition, closePos.value() - streamingPosition);
    if (currentCallValid) {
        ...
        if (parsedArguments.has_value()) {
            ...
            SPDLOG_LOGGER_WARN(llm_calculator_logger,
                "Gemma4 native argument parse failed; refusing executable tool call '{}'.", toolCall.name);
            currentCallValid = false;
...
std::optional<Delta> Gemma4ToolParser::parseChunk(
        ...
        if (currentState == State::Content) {
            ...
            for (const std::string& tagToErase : {TURN_END_TAG, TOOL_RESPONSE_START_TAG}) {
                ...
                while (tagPos != std::string::npos) {
                    content.erase(tagPos, tagToErase.size());
