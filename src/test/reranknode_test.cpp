...
TEST(RerankNodeInitializerTest, MissingModelsReturnsInitializationError) {
    TempDir tempDir;
    mediapipe::CalculatorGraphConfig_Node node;
    node.set_name("missing_rerank");
    node.set_calculator("RerankCalculatorOV");
    mediapipe::RerankCalculatorOVOptions options;
    ...
    EXPECT_TRUE(sidePackets.rerankServableMap.empty());
}
