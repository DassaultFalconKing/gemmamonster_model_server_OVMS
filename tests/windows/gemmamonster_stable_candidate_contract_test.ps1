...
function New-FakeCandidate([string]$Root, [string]$RuntimeProfile) {
    ...
    if ($RuntimeProfile -eq 'maintainer-rc2') {
        $pins = [ordered]@{
            ...
            XGRAMMAR_VERSION = 'v0.2.6'
            XGRAMMAR_COMMIT = 'bc09a30ec10ba30a6c1ab0c79eaeba3ca518d11f'
            XGRAMMAR_MAX_WHITESPACE_CNT = 2
        ...
        $pins = [ordered]@{
            ...
            XGRAMMAR_VERSION = 'v0.1.31'
            XGRAMMAR_COMMIT = ''
            XGRAMMAR_MAX_WHITESPACE_CNT = $null
...
try {
    ...
    $wrongXGrammar = Join-Path $temp 'wrong-xgrammar'
    $sha = New-FakeCandidate -Root $wrongXGrammar -RuntimeProfile 'maintainer-rc2'
    $manifestPath = Join-Path $wrongXGrammar 'manifest.json'
    $manifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding UTF8 | ConvertFrom-Json
    $manifest.dependency_pins.XGRAMMAR_VERSION = 'v0.1.31'
    $manifest | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
    Expect-Failure { & $verifier -CandidateRoot $wrongXGrammar -ExpectedSourceSha $sha -ExpectedRuntimeProfile 'maintainer-rc2' | Out-Null } 'Dependency pin mismatch'
